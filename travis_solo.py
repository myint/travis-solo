...
class Structure(object):
    ...
    def __repr__(self):
        return '%s(%s)' % (
            ...
            ', '.join(('%s=%r' % (key, getattr(self, str(key)))
...
class Configuration(Structure):
    ...
    def prepare_virtualenv(self):
        ...
        try:
            ...
            log_error(e)
            raise Exception(
                'No virtualenv executable found, please install virtualenv')
    ...
    def prepare_environment(self):
        self.environ['PATH'] = ':'.join(
            (join(self.virtualenv_path, 'bin'), self.environ['PATH']))
    ...
    def run(self, jobs=1):
        ...
        success = all(
            result for (configuration,
                        result,
                        _) in results if not configuration.can_fail)
